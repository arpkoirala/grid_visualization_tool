...
def generate_nodes(pandanet,vlevel,colorgradient1,colorgradient2,v_cuttoff_under,v_cuttoff_over,): #labels
    ...
    if vlevel: # if vlevel is True, nodes will be colorscaled according to their vlevel
        for node in pandanet.bus.index:
            pu_level = str(round(vlevels[node],2))
            ...
            if node in ext_grid:
                all_nodes.append({'data':{'id': str(node), 'pu': pu_level }, 'position': {'x': x_coord, 'y': -y_coord}, 'classes': 'ext_grid' + classes_suffix})
            elif isnan(vlevels[node]):
                print('bus ' + str(node) + '= nan')
                all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord}, 'classes': 'bus-nan' + classes_suffix})
            else:
                if vlevels[node] <=1:
                    ...
                    if colorindex < len(colorgradient1):
                        all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord}, 'classes': colorgradient1[colorindex][1:] + classes_suffix})
                    else:
                        all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord},'classes': colorgradient1[len(colorgradient1) -1][1:] + classes_suffix})
                    ...
                    if colorindex < len(colorgradient2):
                        all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord}, 'classes': colorgradient2[colorindex][1:] + classes_suffix})  
                    else:
                        all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord},'classes': colorgradient2[len(colorgradient2) -1][1:] + classes_suffix})
                colorindex = int()
    else:
        for node in pandanet.bus.index:
            pu_level = str(round(vlevels[node],2))
            ...
            if node in ext_grid:
                ...
                all_nodes.append({'data':{'id': str(node), 'pu': pu_level}, 'position': {'x': x_coord, 'y': -y_coord}, 'classes': 'busstandard' + classes_suffix})
    return all_nodes
